...
export class NativeNarrationPlayer {
  protected nativeSession: Promise<number> | null = null;
  protected resolvedSession: number | null = null;
  private pluginListener: PluginListener | null = null;
  private listenerStarted = false;
  protected currentSource: string | null = null;
  private stagedPath: string | null = null;
  private stagedHref: string | null = null;
  protected rate = 1;
  protected userPaused = true;
  protected ended = false;
  private pollTimer: ReturnType<typeof setInterval> | null = null;
  protected cache = { mediaSec: 0, playing: false, at: 0 };
  ...
  /**
   * Load a remote chapter straight into the native AVPlayer — used by the
   * audiobook streaming transport. No staging: AVPlayer fetches the URL with
   * its own range requests, exactly like an HTMLAudioElement would.
   */
  async loadUrl(href: string, startSec = 0): Promise<void> {
    await this.ensureReady();
    this.currentSource = href;
    this.ended = false;
    await invoke('plugin:native-tts|playout_control', {
  ...
  async shutdown(): Promise<void> {
    ...
    if (this.pluginListener) {
      const listener = this.pluginListener;
  ...
  private async stageBlob(href: string, blob: Blob): Promise<string> {
  ...
  async ensureListener(): Promise<void> {
    ...
    try {
      this.pluginListener = await addPluginListener(
        'native-tts',
        'playout_events',
        (event: unknown) => this.onNativeEvent(event as PlayoutEvent),
      );
    } catch (err) {
      this.listenerStarted = false;
      throw err;
  ...
  protected onNativeEvent(event: PlayoutEvent): void {
  ...
  protected async poll(): Promise<void> {
    ...
    try {
      ...
      if (pos.index >= 0) {
        ...
        if (pos.durationMs && pos.durationMs > 0) {
          this.nativeDurationSec = pos.durationMs / 1000;
        }
      } else {
  ...
  /** Duration learned from the native player item, when reported. */
  protected nativeDurationSec: number | null = null;
}
