...
**${_('Exported from Izum StoryFlow')}**: {{ exportDate | date('%Y-%m-%d') }}
  ...
  const markdownPreview = useMemo(() => {
    ...
    if (exportConfig.useCustomTemplate) {
      ...
      if (exportConfig.includeDate) {
        lines.push(
          `**${_('Exported from Izum StoryFlow')}**: ${new Date().toISOString().slice(0, 10)}`,
        );
        lines.push('');
      }
      ...
      <div className='flex flex-col gap-4'>
        ...
        <div className='flex items-center justify-between gap-3'>
          ...
          >
            ...
            <option value='json'>{_('JSON (Izum StoryFlow)')}</option>
          </select>
        </div>

        {isJson && (
          <p className='text-base-content/70 text-xs'>
            {_('A machine-readable file that Izum StoryFlow can import back into any book.')}
          </p>
        )}
