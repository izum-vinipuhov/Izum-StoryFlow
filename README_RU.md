<div align="center">
  ...
  <a href="#get-yandex-token">
    <img src="https://img.shields.io/badge/Получить_токен-Яндекс-FFCC00?style=for-the-badge" alt="Получить токен Яндекса" />
  </a>
</div>
...
- 🔌 **Клиент-сервер** — подключайте приложение к собственному серверу синхронизации
- 🎧 **Стриминг аудиокниг** — слушайте аудиокниги напрямую со своего сервера без скачивания
...
**1. Получите токен Яндекса**

<a id="get-yandex-token"></a>

Для загрузки с Яндекс Книг нужен токен аккаунта Яндекс. Чтобы его получить, запустите скрипт-помощник:

`python get_token.py`

(нужен пакет `pywebview`: `pip install pywebview`)

- Откроется окно входа в аккаунт Яндекс — войдите обычным способом (в том числе через QR-код)
- После входа токен сохранится в `token.txt` рядом со скриптом и выведется в консоль
- Если токен устарел (в приложении ошибка 401) — просто запустите скрипт ещё раз

**2. Укажите токен Яндекса**
...
Зайдите во вход в аккаунт и нажмите **Configure server**:

<p align="center">
  <img src="./data/screenshots/sign_in.jpg" alt="Вход в аккаунт" width="380" />
</p>

Откроется окно, в которое вводится адрес сервера в формате `http://host:port` или `https://host:port`:

<p align="center">
  <img src="./data/screenshots/configure_server.jpg" alt="Настройка сервера" width="620" />
</p>

На собственных серверах по умолчанию включена **общая библиотека**: каждая
книга, загруженная на сервер (включая серверные загрузки «На сервер»),
видна всем аккаунтам, при этом позиции чтения, заметки и настройки
остаются у каждого пользователя своими. Чтобы библиотека была приватной
для каждого загрузившего, задайте `SHARED_LIBRARY=false` в окружении
сервера — подробности в `docker/README.md`.

### Стриминг аудиокниг с сервера

Аудиокниги на сервере можно слушать без скачивания — главы стримятся по
запросу во время прослушивания. Работает для книг, загруженных на сервер,
и для серверных загрузок с Яндекс Книг: откройте аудиокнигу и нажмите
воспроизведение — файлы на устройство не сохраняются.
